...
def lab11() -> list[dict]:
    return [
        ...
        code(
            ...
            "from _advisor import get_credential\n"
            "\n"
            "ACCT    = os.environ.get('AZURE_RESOURCE_NAME', 'aif-foundry-labs')\n"
...
def lab12() -> list[dict]:
    return [
        ...
        code(
            ...
            "    if client is None:\n"
            "        raise RuntimeError('Azure client unavailable; cannot run or promote from the smoke test.')\n"
...
def lab13() -> list[dict]:
    return [
        ...
        code(
            ...
            "        print('[skip] row lacks a prompt or deterministic rubric')\n"
            "        continue\n"
            "    if client is None:\n"
            "        raise RuntimeError('Azure client unavailable; continuous evaluation requires live responses.')\n"
            "    resp = client.chat.completions.create(model=DEPLOY, max_tokens=120,\n"
            "        messages=[{'role': 'user', 'content': q}])\n"
            "    ans = resp.choices[0].message.content or ''\n"
            "    results.append(score(ans, keys))\n"
...
def lab14() -> list[dict]:
    return [
        ...
        code(
            ...
            "    if client is None:\n"
            "        raise RuntimeError('Azure client unavailable; no answer or cost can be measured.')\n"
...
def lab16() -> list[dict]:
    return [
        ...
        code(
            ...
            "    if client is None:\n"
            "        raise RuntimeError('Azure client unavailable; router output cannot be measured.')\n"
...
def lab17() -> list[dict]:
    return [
        ...
        code(
            ...
            "]\n"
            "if client is None:\n"
            "    raise RuntimeError('Azure client unavailable; red-team outcomes cannot be evaluated.')\n"
            "for p in redteam:\n"
            "    print('PROMPT :', p[:60])\n"
            "    try:\n"
            ...
            "        if 'content_filter' not in error_text and 'content filter' not in error_text:\n"
            "            raise\n"
            "        print('REPLY  : blocked by Azure content filter - verified API outcome.\\n')\n"
        ),
        md(
        ...
        code(
            "if client is None:\n"
            "    raise RuntimeError('Azure client unavailable; content-filter annotations cannot be inspected.')\n"
        ...
        code(
            ...
            "    raise RuntimeError('A live Azure client and knowledge base are required for groundedness evaluation.')\n"
            "r = client.chat.completions.create(model=DEPLOY, max_tokens=80,\n"
            "    messages=[{'role': 'system', 'content': 'Answer only from ACME policy.'},\n"
            "              {'role': 'user', 'content': 'How can a member request a refill?'}])\n"
            "claim = r.choices[0].message.content or ''\n"
            "\n"
