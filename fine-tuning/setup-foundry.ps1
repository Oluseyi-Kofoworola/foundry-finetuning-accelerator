...
# Preserve client-branding + tracing vars across re-runs (not derivable from Azure).
$appInsights = ''
$clientSlug  = 'acme'
...
if (Test-Path $envPath) {
    foreach ($line in Get-Content $envPath) {
        if ($line -match '^APPLICATIONINSIGHTS_CONNECTION_STRING=(.*)$') { $appInsights = $Matches[1] }
        elseif ($line -match '^CLIENT_SLUG=(.*)$')    { $clientSlug = $Matches[1] }
        elseif ($line -match '^CLIENT_NAME=(.*)$')    { $clientName = $Matches[1] }
        elseif ($line -match '^FINETUNE_PREFIX=(.*)$'){ $ftPrefix   = $Matches[1] }
    }
}
...
# Application Insights for Foundry Tracing tab
APPLICATIONINSIGHTS_CONNECTION_STRING=$appInsights

CLIENT_SLUG=$clientSlug
CLIENT_NAME=$clientName
FINETUNE_PREFIX=$ftPrefix
"@
