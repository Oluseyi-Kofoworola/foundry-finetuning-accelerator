...
async function main() {
  ...
  app.post('/api/realtime/session', async (req, res) => {
    try {
      const { scenarioId, consentGranted = false } = req.body as {
        scenarioId?: string;
      ...
      if (config.security.requireConsent && !consentGranted) {
        return res.status(200).json({
          ...
          data: {
            ...
            scenario: {
              ...
              consentMessage: 'This call may be recorded for quality and security. Continue?',
            },
          },
        });
      }
      ...
      if (config.security.requireConsent) {
        await sessionManager.recordConsent(session.id);
      }
