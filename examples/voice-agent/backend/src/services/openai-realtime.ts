...
export class OpenAIRealtimeService extends EventEmitter {
  ...
  private async initializeSession(): Promise<void> {
    ...
    const tools = getOpenAIToolDefinitions(this.scenario.enabledTools);
  ...
  private async handleFunctionCall(event: any): Promise<void> {
    ...
    try {
      ...
      if (!session || session.status !== 'active' || !session.consentGiven) {
        throw new Error('Session is inactive, expired, or awaiting consent');
      }
      ...
      const context: ToolContext = {
        ...
        allowedTools: this.scenario.enabledTools,
        timestamp: new Date(),
