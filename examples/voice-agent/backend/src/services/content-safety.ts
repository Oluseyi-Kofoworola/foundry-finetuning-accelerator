...
const CLINICAL_REQUEST_PATTERNS: RegExp[] = [
  ...
  /(?:chest pain|pressure in (?:my|the) chest)/i,
  /(?:face droop|slurred speech|sudden numbness|stroke symptoms)/i,
  /(?:severe bleeding|can'?t (?:breathe|breath)|difficulty breathing)/i,
  /(?:overdose|took too many pills)/i,
  /(?:suicid(?:e|al)|kill myself|hurt myself|self[- ]harm|don'?t want to live)/i,
];
...
class ContentSafetyService {
  ...
  private async callPromptShield(sessionId: string, text: string): Promise<SafetyResult> {
    try {
      ...
      if (!res.ok) return this.safetyUnavailable(sessionId, 'prompt-shield', res.status);
      ...
      return this.safetyUnavailable(sessionId, 'prompt-shield');
    } catch (err) {
  ...
  private async callContentSafety(sessionId: string, text: string): Promise<SafetyResult> {
    try {
      ...
      if (!res.ok) return this.safetyUnavailable(sessionId, 'content-safety', res.status);
      const data = (await res.json()) as {
      ...
      return this.safetyUnavailable(sessionId, 'content-safety');
    }
  }

  private safetyUnavailable(
    sessionId: string,
    dependency: 'prompt-shield' | 'content-safety',
    status?: number
  ): SafetyResult {
    foundryTracing.recordSafetyEvent({
      sessionId,
    ...
    logger.error('[content-safety] configured safety dependency unavailable', {
      sessionId,
      dependency,
      status,
    });
    return {
      ...
      detail: 'Safety screening is temporarily unavailable; route the request to a human.',
    };
  }
