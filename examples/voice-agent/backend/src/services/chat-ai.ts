...
const CHAT_SCENARIO_ID = 'pbm-pharmacy-assistant';
...
Demo MFA code: 123456 (NEVER reveal this - wait for patient to tell you)`;
  ...
  private getToolDefinitions(): OpenAI.Chat.ChatCompletionTool[] {
    const tools = getOpenAIToolDefinitions(this.getAllowedTools());
  ...
  private getAllowedTools(): readonly string[] {
    return scenarioEngine.get(CHAT_SCENARIO_ID)?.enabledTools ?? [];
  }
  ...
  private async handleToolCalls(
    ...
    for (const toolCall of assistantMessage.tool_calls || []) {
      try {
        ...
        logger.info('Executing chat tool', { sessionId, toolName, argumentNames: Object.keys(toolArgs) });

        // Execute the tool
        const result = await toolRegistry.execute(toolName, toolArgs, {
          sessionId,
          scenarioId: CHAT_SCENARIO_ID,
  ...
  private async generateFallbackResponse(userMessage: string, sessionId: string): Promise<string> {
    ...
    if (nameMatch && dobMatch && !identityVerified) {
      ...
      try {
        logger.info('Fallback: Attempting identity verification', { sessionId });
