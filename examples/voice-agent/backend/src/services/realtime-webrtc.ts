...
export async function mintEphemeralSession(
  ...
  const sessionConfig = {
    ...
    session: {
      ...
      tools: getOpenAIToolDefinitions(scenario.enabledTools),
      tool_choice: 'auto',
...
export async function executeToolForSession(
  ...
  if (!session || session.status !== 'active' || !session.consentGiven) {
    return { success: false, data: null, error: 'Session is inactive, expired, or awaiting consent.' };
  }

  const scenario = scenarioEngine.get(session.scenarioId);
  if (!scenario) {
    return { success: false, data: null, error: 'Session scenario is unavailable.' };
  }
  ...
  const context: ToolContext = {
    ...
    allowedTools: scenario.enabledTools,
    timestamp: new Date(),
