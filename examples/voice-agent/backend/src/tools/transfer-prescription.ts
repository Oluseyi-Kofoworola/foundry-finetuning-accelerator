...
>({
  ...
  The first call stages the exact transfer. Read it back to the patient, wait for explicit confirmation,
  then repeat the same call with the returned confirmationToken.
  ...
  parameters: {
    ...
    properties: {
      ...
      confirmationToken: {
        type: 'string',
        description: 'One-time token returned by the staged transfer after the patient explicitly confirms it',
      },
    },
