...
// =============================================================================
// COMPREHENSIVE MEDICAL RECORDS - Alex Example (MEM-001)
// =============================================================================
...
// Alex Example's Complete Medical Record
...
>({
  ...
  handler: async (args, context): Promise<ToolResult<MedicalRecordsResult>> => {
    ...
    if (cachedRecords && cachedRecords.found) {
      logger.debug('[Cache] Medical records hit', { sessionId, section });
    ...
    if (!record) {
      return {
        ...
        data: {
          found: false,
          message: `Medical records for member ${memberId} are not available. Available demo patients: Alex Example (MEM-001), Robert Martinez (MEM-002), Emily Chen (MEM-003), James Wilson (MEM-004).`,
        },
      };
    }
...
>({
  ...
  handler: async (args, context): Promise<ToolResult<AppointmentSlotsResult>> => {
    ...
    const roster = [
      { provider: 'Dr. Taylor Example', facility: 'Acme Health — Chesterfield', modality: 'in-person' as const },
      { provider: 'Dr. Taylor Example', facility: 'Acme Health Telehealth', modality: 'telehealth' as const },
...
>({
  ...
  handler: async (args, context): Promise<ToolResult<AppointmentResult>> => {
    ...
    const assignedProvider = provider || 'Dr. Taylor Example';
