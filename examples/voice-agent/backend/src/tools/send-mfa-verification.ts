...
function challengeKey(sessionId: string, memberId: string): string {
...
>({
  ...
  handler: async (args, context): Promise<ToolResult<SendMFAResult>> => {
    ...
    pendingMFACodes.set(challengeKey(context.sessionId, memberId), { code, expiresAt, action });
...
>({
  ...
  handler: async (args, context): Promise<ToolResult<VerifyMFAResult>> => {
    ...
    if (cachedMFA && cachedMFA.verified && cachedMFA.canProceed) {
      logger.debug('[Cache] MFA verification hit', { sessionId });
      return { success: true, data: cachedMFA };
    }

    await new Promise(resolve => setTimeout(resolve, 300));

    const pendingKey = challengeKey(sessionId, memberId);
    const pending = pendingMFACodes.get(pendingKey);
    ...
    if (new Date() > pending.expiresAt) {
      pendingMFACodes.delete(pendingKey);
      return {
        success: true,
        data: {
          verified: false,
