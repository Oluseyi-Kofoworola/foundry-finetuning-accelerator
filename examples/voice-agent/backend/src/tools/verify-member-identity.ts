...
const MOCK_MEMBERS = new Map([
  // Patient 1: Alex Example - 38-year-old with Type 2 Diabetes & Hypertension
  ['MEM-001', {
    memberId: 'MEM-001',
    firstName: 'Sarah',
    lastName: 'Johnson',
...
>({
  ...
  parameters: {
    ...
    properties: {
      ...
      fullName: {
        type: 'string',
        description: "Member's full name as a single string (use this if firstName/lastName aren't separately available, e.g. 'Alex Example')",
      },
      dateOfBirth: {
        type: 'string',
  ...
  handler: async (args, context): Promise<ToolResult<VerificationResult>> => {
    ...
    if (memberId) {
      ...
      if (cachedIdentity && cachedIdentity.verified) {
        logger.debug('[Cache] Identity verification hit', { sessionId });
    ...
    logger.debug('[verify_member_identity] attempt', {
      sessionId,
      providedFactors: Object.entries({ firstName, lastName, fullName, dateOfBirth, lastFourSSN, zipCode, memberId })
        .filter(([, value]) => !!value)
        .map(([key]) => key),
    });
