...
const BACKEND_HOST = (import.meta as { env?: Record<string, string> }).env?.VITE_BACKEND_HOST
...
export class WebRtcRealtimeService {
   ...
   */
  async connect(scenarioId?: string, consentGranted = false): Promise<ConnectResult> {
    if (this.connectInFlight) return this.connectInFlight;
    if (this.pc && this.sessionId && this.cachedScenario) {
      return { sessionId: this.sessionId, scenario: this.cachedScenario };
    }
    this.connectInFlight = this._doConnect(scenarioId, consentGranted).finally(() => {
      this.connectInFlight = null;
    });
    return this.connectInFlight;
  }

  private async _doConnect(scenarioId?: string, consentGranted = false): Promise<ConnectResult> {
    ...
    const mintResp = await fetch(`${apiBase}/api/realtime/session`, {
      ...
      body: JSON.stringify({ scenarioId, consentGranted }),
    });
    if (!mintResp.ok) {
    ...
    if (minted.pendingConsent) {
      this.emit('session.created', { sessionId: null, scenario: minted.scenario });
      return { sessionId: null, scenario: minted.scenario };
    }
    if (!minted.sessionId || !minted.clientSecret || !minted.webrtcUrl || !minted.sessionConfig) {
      throw new Error('Realtime session response omitted required connection credentials');
    }
    const sessionConfig = minted.sessionConfig;
