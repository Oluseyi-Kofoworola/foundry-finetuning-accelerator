...
export function useVoiceAgentWebRTC(): UseVoiceAgentReturn {
  ...
  useEffect(() => {
    ...
    unsubs.push(webrtcRealtimeService.on('session.created', (evt) => {
      ...
      setVoiceState(payload.scenario.requiresConsent ? 'idle' : 'listening');
      setIsAutoListening(!payload.scenario.requiresConsent);
    }));
  ...
  const provideConsent = useCallback(() => {
    setConnectionStatus('connecting');
    setError(null);
    void webrtcRealtimeService
      .connect(pendingScenarioRef.current ?? session.scenarioId ?? undefined, true)
      .catch((err) => {
