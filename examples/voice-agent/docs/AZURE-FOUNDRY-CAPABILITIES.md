# Microsoft Foundry Capability Map — Voice Agent

This codebase is deliberately structured so that each Microsoft Foundry capability
...
| Agent IDs per scenario | `backend/agent-ids.json`, `backend/foundry-agent-ids.json` (generated locally, gitignored) |
| Agent definitions (catalog) | `foundry-agents.json` (generated locally, gitignored) |
